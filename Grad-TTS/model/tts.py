...
class GradTTS(BaseModule):
    ...
    def forward(self, x, x_lengths, n_timesteps, temperature=1.0, stoc=False, spk=None, length_scale=1.0):
        ...
        y_mask = sequence_mask(torch.tensor([y_max_length]), y_max_length_).unsqueeze(1).to(x_mask) #(1, 1, 200)

        mu_y = torch.nn.functional.pad(mu_y, (0, y_max_length_ - y_max_length), mode='constant', value=0)
    ...
    def compute_loss(self, x, x_lengths, y, y_lengths, spk=None, out_size=None):
        ...
        labels = create_eos_labels(y, y_lengths).to(self.device)
        ...
        attn_mask = y_mask_.unsqueeze(-1) * x_mask.unsqueeze(2) # (1, 1, 200, 1) * (1, 1, 1, 55) = (1, 1, 200, 55)
        
        mu_y, eos_loss = self.attention_align.compute_encoder_output_n_eos_loss(mu_x.transpose(1, 2), attn_mask, decoder_input, tgt_mask.unsqueeze(1), y_mask, labels)
        ...
        if not isinstance(out_size, type(None)):
            ...
            labels_cut = torch.zeros(y.shape[0], out_size, dtype=y.dtype, device=y.device) #(16, 80, 172)
            mu_y_cut_lengths = []
            # y: (16, 80, 812) out_offset: (16)
            for i, (mu_y_, y_, out_offset_, labels_) in enumerate(zip(mu_y, y, out_offset, labels)):
                ...
                y_cut[i, :, :mu_y_cut_length] = y_[:, cut_lower:cut_upper]
