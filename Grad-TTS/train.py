    ...
    for epoch in range(1, n_epochs + 1):
        ...
        with tqdm(loader, total=len(train_dataset)//batch_size) as progress_bar:
            for batch_idx, batch in enumerate(progress_bar):
                ...
                dur_loss, prior_loss, diff_loss, attention_loss = model.compute_loss(x, x_lengths,
                                                                     y, y_lengths,
                                                                     out_size=out_size)
                # loss = sum([dur_loss, prior_loss, diff_loss])
                loss = sum([prior_loss, diff_loss, attention_loss])
                loss.backward()
                ...
                logger.add_scalar('training/diffusion_loss', diff_loss.item(),
                                  global_step=iteration)
                logger.add_scalar('training/attention_loss', attention_loss.item(),
                                  global_step=iteration)
                ...
                if batch_idx % 5 == 0:
                    msg = f'Epoch: {epoch}, iteration: {iteration} | dur_loss: {dur_loss.item()}, prior_loss: {prior_loss.item()}, diff_loss: {diff_loss.item()}, attention_loss: {attention_loss.item()}'
